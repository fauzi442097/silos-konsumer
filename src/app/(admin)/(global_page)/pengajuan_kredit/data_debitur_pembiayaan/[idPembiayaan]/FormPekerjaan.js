...
const FormPekerjaan = ({ data, statePekerjaan, register, errors, control, setValue, getValue }) => {
    ...
    const useGetPekerjaan = () => {
        ...
        if (getPekerjaan.isSuccess) {
            ...
            dataPekerjaan.map((item) => {
                return arrPekerjaan.push({ value: item.idPekerjaan, label: item.nmPekerjaan + ' - Max.Umur (' + item.masaKerjaUmur + ' Tahun)' })
            })
        }
    ...
    useEffect(() => {
        ...
        if (dataNasabah) {
            ...
            setValue('nama_kantor', dataNasabah[0].nasabah.tempat_kerja, { shouldDirty: true, shouldValidate: true, shouldTouched: true } );
            setValue('no_telp_kantor', dataNasabah[0].nasabah.office_phone, { shouldDirty: true, shouldValidate: true, shouldTouched: true } );
            setValue('jabatan', dataNasabah[0].nasabah.jabatan, { shouldDirty: true, shouldValidate: true, shouldTouched: true } );
    ...
    return (
        <>
            ...
            <div className="flex flex-row justify-start gap-4 w-full md:flex-nowrap flex-wrap my-4 mb-7" style={{ gap: "30px" }}>
                ...
                <div style={{ width: "325px" }}>
                    <label className='block mb-3'> Tempat Kerja Nasabah </label>
                    <Controller
                        control={control}
                        name="nama_kantor"
                        id="nama_kantor"
